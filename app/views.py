...
def checkcontract(request):
    try:
        ...
        url = ("http://210.107.78.156:80/keyHistory/%s" % cid)
        ...
        try:
            if otp == nowotp and Member.objects.get(user_id=share_user):
                if history[0]['Value']['ci']['To'][11:] == user_id:
                    url = (
                            'http://210.107.78.156:80/add_LCR/' + contract_id + '- importer: ' + user_id + '- bank: ' + share_user + '- letter of credit request: ' + hash)
...
def share2(request):
    try:
        ...
        try:
            if otp == nowotp and Member.objects.get(user_id=share_user):
                url = 'http://210.107.78.156:80/add_CI/' + contract_id + '- Exporter: ' + user_id + '- importer: ' + share_user + '- commercial invoice: ' + hash
                response = requests.post(url)
                res = response.text
        ...
        try:
            if otp == nowotp and Member.objects.get(user_id=share_user):
                if history[2]['Value']['lc']['To'][11:] == user_id:
                    url = 'http://210.107.78.156:80/add_SR/' + contract_id + '- exporter: ' + user_id + '- shipper: ' + share_user + '- shipping request: ' + hash
                    response = requests.post(url)
                    res = response.text
        ...
        try:
            if otp == nowotp and Member.objects.get(user_id=share_user) and Member.objects.get(user_id=share_user2):
                if history[1]['Value']['lcr']['To'][7:] == user_id:
                    url = 'http://210.107.78.156:80/add_LC/' + contract_id + '- bank: ' + user_id + '- exporter: ' + share_user2 + '- letter of credit: ' + hash
                    response = requests.post(url)
                    res = response.text
                    result_dict = {}
        ...
        try:
            if otp == nowotp and Member.objects.get(user_id=share_user) and Member.objects.get(user_id=share_user2):
                if history[3]['Value']['sr']['To'][10:] == user_id:
                    url = 'http://210.107.78.156:80/add_BL/' + contract_id + '- shipper: ' + user_id + '- importer: ' + share_user + '- bill of landing: ' + hash
                    response = requests.post(url)
                    res = response.text
        ...
        try:
            if otp == nowotp and Member.objects.get(user_id=share_user) and Member.objects.get(user_id=share_user2):
                if history[4]['Value']['bl']['from'][10:] == user_id:
                    url = 'http://210.107.78.156:80/add_DO/' + contract_id + '- shipper: ' + user_id + '- importer: ' + share_user + '- delivery order: ' + hash
                    response = requests.post(url)
                    res = response.text
                    result_dict = {}
